...
norm_cols <- c("Food_nose_pokes", "Wheel_cycles")

no_wheel_ids <- c("1789", "1836")
...
scaled_list <- lapply(names(group_pairs), function(ctrl) {
  ...
  # Compute vehicle means, excluding broken-wheel mice for Wheel_cycles
  ctrl_means <- colMeans(ctrl_df[, cols_present], na.rm = TRUE)
  if ("Wheel_cycles" %in% cols_present) {
    ctrl_means["Wheel_cycles"] <- mean(
      ctrl_df$Wheel_cycles[!ctrl_df$id %in% no_wheel_ids], na.rm = TRUE
    )
  }
...
main_behaviors <- c("Food_nose_pokes", "Wheel_cycles")
...
distance_data <- distance_data %>%
  filter(
    !(group %in% c("saline_orfo", "orfo"))       | id %in% keep_ids$orfo,
    !(group %in% c("dan_sd", "veh_sd")) | id %in% keep_ids$dan,
    !(group %in% c("lira_sd", "saline_lira")) | id %in% keep_ids$lira,
    !(group %in% c("LiCl", "saline_licl")) | id %in% keep_ids$licl
