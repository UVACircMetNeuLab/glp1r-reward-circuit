...
behavior_data <- read.csv("behavior_time.csv") %>% # behavior time 
...
transitions <- read_csv("normalized_transitions.csv") %>% #norm transitions 
  filter(group %in% c("dan_sd", "lira_sd", "orfo", "LiCl", "fed"))
...
bouts <- read_csv("bouts.csv") %>% #bouts
...
# Add back id and group
pc_scores$id <- combined_scaled$id
pc_scores$group <- combined_scaled$group
